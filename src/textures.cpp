    ...
    mTextureSize{ 0, 0 }
{
    ...
    unsigned char* pixmap = stbi_load(filename.c_str(), &width, &height, &numChannels, 0);
    if (!pixmap) {
...
{
    assert(point.x < mTextureSize.width && point.y < mTextureSize.height && id < mTextureCount);
    return mImage[point.x + id * mTextureSize.width + point.y * mImageSize.width];
}

std::vector<uint32_t> TextureSet::getScaledColumn(const size_t id, const size_t xCoordinate, const size_t height) const
{
